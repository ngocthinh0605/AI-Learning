 ...
 *
 * Props:
 *   exercise     {Object}   - { type, prompt, options, answer, explanation }
...
export default function TrainingExerciseCard({ exercise, onAnswer, onNext, isLast }) {
  ...
  const { type, prompt, options = [], answer, explanation } = exercise;
    ...
    <div className="bg-gray-800 rounded-2xl border border-white/10 p-6 space-y-4">
      ...
      <div className="space-y-2">
        {options.map((opt, i) => {
          ...
          const correct    = answer?.trim().toLowerCase() === opt.trim().toLowerCase();
      ...
      {revealed && (
        <div className={`flex items-start gap-2 p-3 rounded-xl text-sm
          ...
          <span>{explanation || (isCorrect ? "Correct!" : `Correct answer: ${answer}`)}</span>
        </div>
      )}
